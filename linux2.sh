...
mysql -e "CREATE DATABASE IF NOT EXISTS $db_wordpress default character set utf8 collate utf8_unicode_ci;"
mysql -e "CREATE USER IF NOT EXISTS '$user'@'localhost' IDENTIFIED BY '$pass';"
mysql -e "GRANT ALL ON $db_wordpress.* to '$user'@'localhost' IDENTIFIED BY '$pass';"
